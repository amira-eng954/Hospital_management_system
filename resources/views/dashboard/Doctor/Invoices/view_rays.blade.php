...
@section('content')
    ...
    <div class="demo-gallery">
        <ul id="lightgallery" class="list-unstyled row row-sm pr-0">
            ...
            @foreach($rays->image as $image)

            <li class="col-sm-6 col-lg-4" data-responsive="{{URL::asset('storage/rays/'.$image->image_name)}}" data-src="{{URL::asset('storage/rays/'.$image->image_name)}}">
                <a href="#">
                    <img width="50px" height="350px" class="img-responsive" src="{{URL::asset('storage/rays/'.$image->image_name)}}" alt="NoImg">
                </a>
            </li>
            @endforeach
        </ul>
